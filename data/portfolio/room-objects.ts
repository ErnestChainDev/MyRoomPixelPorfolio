import { asset2Layer, layer } from "./assets";
import { profile } from "./profile";
import type { RoomObjectConfig } from "./types";

export const roomObjects: RoomObjectConfig[] = [
  {
    id: "contact-top-note",
    label: "Contact",
    position: { left: 5.82, top: 22.15 },
    width: 5.1,
    height: 8.7,
    zIndex: 12,
    hotspotOnly: true,
  },
  {
    id: "contact-bottom-note",
    label: "Contact",
    image: layer("contact2.png"),
    position: { left: 6.05, top: 34.15 },
    targetScene: "contact",
    width: 3.44,
    zIndex: 12,
    zoom: { scale: 1.3, x: "19%", y: "9%" },
    cue: "left",
    shortcutGroup: "contact",
  },
  {
    id: "about-note",
    label: "About",
    image: layer("about.png"),
    position: { left: 13.55, top: 33.14 },
    targetScene: "about",
    width: 3.3,
    zIndex: 12,
    zoom: { scale: 1.3, x: "16%", y: "9%" },
    cue: "right",
  },
  {
    id: "folder",
    label: "Experience",
    image: asset2Layer("Folder.png"),
    position: { left: 14.24, top: 22.33 },
    targetScene: "experience",
    width: 2.84,
    zIndex: 13,
    zoom: { scale: 1.3, x: "17%", y: "14%" },
    cue: "right",
    shortcutGroup: "experience",
  },
  {
    id: "portfolio-screen",
    label: "Computer",
    image: layer("PC.png"),
    position: { left: 14.65, top: 52.64 },
    width: 22.41,
    zIndex: 10,
    zoom: { scale: 1.22, x: "11%", y: "4%" },
    cue: "top",
    shortcutGroup: "portfolio",
    modal: {
      kind: "computer",
      eyebrow: "Main workstation",
      title: "Computer",
      summary:
        "Primary workspace for software development, AI projects, testing, and full-stack application development.",
      description:
        "My main workstation used for building web applications, AI-powered systems, software testing, and exploring new technologies.",
      stats: [
        { label: "Role", value: "Developer" },
        { label: "Focus", value: "AI & QA" },
        { label: "Stack", value: "React + FastAPI" },
      ],
      highlights: [
        "AI-powered software projects",
        "Full-stack web development portfolio",
        "Quality assurance and testing experience",
      ],
      actions: [
        { label: "Open Work", scene: "projects", variant: "primary" },
        {
          label: "GitHub",
          href: "https://github.com/ErnestChainDev",
          variant: "secondary",
        },
      ],
    },
  },
  {
    id: "ferrari",
    label: "Ferrari model",
    image: asset2Layer("Ferrari.png"),
    position: { left: 69.37, top: 11.95 },
    width: 10.97,
    zIndex: 13,
  },
  {
    id: "mercedes",
    label: "Mercedes model",
    image: asset2Layer("Mercedes.png"),
    position: { left: 80.4, top: 11.91 },
    width: 10.44,
    zIndex: 13,
  },
  {
    id: "aston-martin",
    label: "Aston Martin model",
    image: asset2Layer("AstonMartin.png"),
    position: { left: 69.16, top: 24.19 },
    width: 10.9,
    zIndex: 13,
  },
  {
    id: "redbull",
    label: "Red Bull model",
    image: asset2Layer("Redbull.png"),
    position: { left: 80.45, top: 24.12 },
    width: 10.76,
    zIndex: 13,
  },
  {
    id: "red-motor",
    label: "Learner's AI",
    image: asset2Layer("RedMotor.png"),
    position: { left: 69.58, top: 36.13 },
    externalUrl: "https://learners-ai.vercel.app/",
    width: 9.48,
    zIndex: 13,
  },
  {
    id: "projects-note",
    label: "Work",
    image: layer("Projects.png"),
    position: { left: 9.57, top: 26.5 },
    targetScene: "projects",
    width: 4.23,
    zIndex: 12,
    zoom: { scale: 1.28, x: "18%", y: "12%" },
    cue: "bottom",
    shortcutGroup: "portfolio",
  },
  {
    id: "store",
    label: "Services",
    image: layer("Store.png"),
    position: { left: 53.68, top: 79.49 },
    targetScene: "services",
    width: 15.16,
    zIndex: 12,
    zoom: { scale: 1.18, x: "-2%", y: "-7%" },
    cue: "top",
    shortcutGroup: "services",
  },
  {
    id: "green-moto",
    label: "TruthFinder",
    image: layer("GreenMoto.png"),
    position: { left: 69, top: 53.91 },
    externalUrl: "https://truth-finder-wine.vercel.app/",
    width: 10.23,
    zIndex: 12,
  },
  {
    id: "printer",
    label: "3D printer",
    image: asset2Layer("Printer.png"),
    position: { left: 80.36, top: 48.18 },
    width: 9.3,
    zIndex: 13,
    zoom: { scale: 1.24, x: "-14%", y: "2%" },
    modal: {
      kind: "printer",
      eyebrow: "Prototype bay",
      title: "QA & Testing Lab",
      summary:
        "Software testing workspace focused on validation, debugging, quality assurance, and application reliability.",
      description:
        "A dedicated area representing quality assurance practices including testing, bug tracking, defect reporting, and software validation.",
      stats: [
        { label: "Testing", value: "Manual" },
        { label: "Tools", value: "Postman" },
        { label: "QA", value: "STLC" },
      ],

      highlights: [
        "Manual and functional testing",
        "Bug reporting and defect tracking",
        "Software quality assurance workflows",
      ],
      actions: [
        {
          label: "Start Build",
          href: "mailto:ernestchaindev@gmail.com?subject=Prototype%20project",
          variant: "primary",
        },
      ],
    },
  },
  {
    id: "red-moto-bottom",
    label: "GitHub Views",
    image: asset2Layer("RedMotoBottom.png"),
    position: { left: 68.71, top: 66.8 },
    externalUrl: "https://github-profile-views-two.vercel.app/",
    width: 8.88,
    zIndex: 13,
  },
  {
    id: "box",
    label: "Storage box",
    image: asset2Layer("Box.png"),
    position: { left: 66.32, top: 77.02 },
    width: 4.37,
    zIndex: 13,
  },
  {
    id: "motor",
    label: "Motorcycle",
    image: layer("Motor.png"),
    position: { left: 82.6, top: 78.45 },
    width: 31.96,
    zIndex: 12,
    zoom: { scale: 1.17, x: "-15%", y: "-8%" },
    modal: {
      kind: "motor",
      eyebrow: "Garage file",
      title: "Career Journey",
      summary:
        "A timeline representing my journey from Computer Science student to Software Developer, QA Engineer, and aspiring AI Engineer.",

      description:
        "Showcases academic achievements, technical growth, certifications, internship experience, and future career goals.",

      stats: [
        { label: "Degree", value: "BSCS" },
        { label: "Graduate", value: "2026" },
        { label: "Goal", value: "AI Engineer" },
      ],

      highlights: [
        "Computer Science Graduate",
        "DICT Internship Experience",
        "Software Development & QA Background",
      ],
    },
  },
  {
    id: "profile-head",
    label: "LinkedIn",
    image: "/head.png",
    position: { left: 96, top: 91.6 },
    externalUrl: profile.linkedin,
    width: 8,
    zIndex: 15,
    baseVisible: true,
  },
  {
    id: "books-shelf",
    label: "Skills",
    image: asset2Layer("Books.png"),
    position: { left: 93.54, top: 48.89 },
    targetScene: "skills",
    width: 11.51,
    zIndex: 13,
    zoom: { scale: 1.2, x: "-15%", y: "0%" },
    cue: "left",
    shortcutGroup: "skills",
    mobileLabel: "Skills",
  },
  {
    id: "code-sign",
    label: "GitHub",
    image: layer("Tablo.png"),
    position: { left: 93.55, top: 12.37 },
    externalUrl: "https://github.com/ErnestChainDev",
    width: 10.33,
    zIndex: 12,
  },
  {
    id: "chair",
    label: "YouTube",
    image: layer("Koltuk.png"),
    position: { left: 39.42, top: 73.24 },
    externalUrl: "https://www.youtube.com/watch?v=N6sIbJDWZ9s&list=RDN6sIbJDWZ9s&start_radio=1",
    width: 16.12,
    zIndex: 12,
    shortcutGroup: "youtube",
  },
  {
    id: "youtube-plaque-top",
    label: "YouTube plaque",
    image: layer("youtube2.png"),
    position: { left: 96.24, top: 31.77 },
    width: 3.48,
    zIndex: 12,
  },
  {
    id: "youtube-plaque-side",
    label: "YouTube plaque",
    image: layer("Youtube1.png"),
    position: { left: 98.01, top: 44.04 },
    width: 3.98,
    zIndex: 12,
  },
  {
    id: "home-icon",
    label: "Home",
    image: asset2Layer("Home.png"),
    position: { left: 47.48, top: 97.88 },
    action: "home",
    width: 1.35,
    zIndex: 13,
    shortcutGroup: "home",
    mobileLabel: "Home",
  },
  {
    id: "menu-icon",
    label: "Menu",
    image: asset2Layer("Menu.png"),
    position: { left: 49.98, top: 97.88 },
    targetScene: "menu",
    width: 1.38,
    zIndex: 13,
    shortcutGroup: "menu",
    mobileLabel: "Menu",
  },
  {
    id: "mic-icon",
    label: "Music",
    image: asset2Layer("Mic.png"),
    position: { left: 52.52, top: 97.85 },
    action: "music",
    width: 1.35,
    zIndex: 13,
    shortcutGroup: "music",
    mobileLabel: "Music",
  },
  {
    id: "mac",
    label: "Mac",
    image: layer("mac.png"),
    position: { left: 30.95, top: 53.42 },
    width: 7.56,
    zIndex: 12,
    zoom: { scale: 1.28, x: "6%", y: "2%" },
    cue: "bottom",
    modal: {
      kind: "mac",
      eyebrow: "Portable workspace",
      title: "Mac",
      summary:
        "Workspace dedicated to AI development, machine learning experiments, and intelligent application design.",
      description:
        "Used for researching AI technologies, building recommendation systems, experimenting with machine learning models, and developing intelligent software solutions.",
      stats: [
        { label: "Focus", value: "AI" },
        { label: "ML", value: "Learning" },
        { label: "Goal", value: "Engineer" },
      ],

      highlights: [
        "Machine Learning Projects",
        "AI Application Development",
        "Recommendation Systems",
      ],
    },
  },
  {
    id: "coffee",
    label: "Coffee",
    image: layer("Coffe.png"),
    position: { left: 13.37, top: 69.73 },
    width: 2.88,
    zIndex: 12,
  },
  {
    id: "desk-figure",
    label: "Desk figure",
    image: layer("Biblo1.png"),
    position: { left: 17.29, top: 61.2 },
    width: 2.13,
    zIndex: 12,
  },
  {
    id: "desk-toy",
    label: "Desk toy",
    image: layer("Biblo2.png"),
    position: { left: 10.44, top: 67.09 },
    width: 2.2,
    zIndex: 12,
  },
  {
    id: "plant",
    label: "Plant",
    image: layer("Cicek.png"),
    position: { left: 91.69, top: 43.39 },
    width: 7.03,
    zIndex: 12,
  },
];
